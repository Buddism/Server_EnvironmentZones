...
function Environment(%isClientEnv)
{
...
function Environment::copyFrom(%this, %other)
{
	...
	} else if(isObject(%this.sunLight))
		%this.sunLight.delete();
	...
	} else if(isObject(%this.dayCycle))
		%this.dayCycle.delete();
	...
	} else if(isObject(%this.groundPlane))
		%this.groundPlane.delete();
	...
	} else if(isObject(%this.waterPlane))
		%this.waterPlane.delete();
	...
	if(%this.isClientEnv)
		%lastCurrent = $currentEnvironment;
	...
	if(%this.isClientEnv)
		$currentEnvironment = %lastCurrent;
}
...
function GameConnection::setEnvironment(%this, %zoneEnvironment)
{
	...
	if(!isObject(%this.currentEnvironment))
	{
		...
		%newEnv = Environment(%isClientEnv = true);
		%newEnv.client = %this;
		%this.currentEnvironment = %newEnv;
	}

	// Swap a clients current environment with this one
	if(%this.currentEnvironment.zoneEnvironment == %zoneEnvironment)
		return;
