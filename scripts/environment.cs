...
//this is for zone environments not client environments
...
function Environment::postEditCheck(%this, %varType, %value)
{
	...
	for(%i = 0; %i < ClientGroup.getCount(); %i++)
	{
		...
		if(%clientEnv.zoneEnvironment == %this)
		{
			if(%varType $= "")
			{
				%clientEnv.setClientEnv(%this);
			} else {
				if(%clientEnv.SetVar(%varType, %value, %this) !$= "")
					%clientEnv.setClientEnv(%this);
			}
		}
	}
...
function GameConnection::setEnvironment(%this, %zoneEnvironment)
{
	...
	%this.currentEnvironment.TimeTransition(%zoneEnvironment, 1000); //1000 ms
